...
trained fly plays live against three rival flies, dealt by a praying mantis. Vesper's moves are a
real forward pass through the 4,010 weights in the browser, with her softmax output and
optimal-play agreement shown on the rail as she plays. Dizzy, at the same table, is this same
architecture trained by the specification's broken loop, busting every hand.
...
policy on all 280 states (0 action mismatches). See [`casino/README.md`](casino/README.md).
