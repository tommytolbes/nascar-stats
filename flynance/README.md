...
trained fly plays live against four rival flies, dealt by a praying mantis. Vesper's moves are a
...
Three of the other seats are the experiment's findings made watchable. **Iris** is the one-hot
ablation — the same mushroom body, shown the dealer's upcard as ten separate inputs instead of one
scalar — so the encoding result (88.4% → 92.4% agreement) can be observed hand by hand rather than
read off a table. **Dizzy** is the same architecture trained by the specification's broken loop,
busting every hand.
...
policy on all 280 states for *both* networks (0 action mismatches).
